...
def _base_tweet_qs(user=None):
    qs = Tweet.objects.select_related("user").annotate(
    ...
    if user is not None and user.is_authenticated:
        ...
        qs = qs.annotate(
            is_liked=Exists(user_model.objects.filter(liked_tweets=OuterRef("pk"), pk=user.pk)),
            is_bookmarked=Exists(
                user_model.objects.filter(bookmarked_tweets=OuterRef("pk"), pk=user.pk)
            ),
        )
    else:
        qs = qs.annotate(
...
def subscriptions_feed_qs(*, user):
    return _base_tweet_qs(user).filter(user__in=user.following.all()).order_by("-created_at", "-id")
...
def bookmarks_qs(*, user):
    return _base_tweet_qs(user).filter(bookmarks=user).order_by("-created_at", "-id")


def tweet_with_comments(*, slug, user=None):
    qs = _base_tweet_qs(user).prefetch_related(
        Prefetch(
            "comments", queryset=Comment.objects.select_related("user").order_by("-created_at")
        )
    )
